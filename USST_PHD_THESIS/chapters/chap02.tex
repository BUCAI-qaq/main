...
图~\ref{fig:System_Block_Diagram} 展示了本文所构建的全身动作模仿系统整体框架。系统主要由动作数据获取模块、人体姿态估计模块、动作重定向模块以及机器人执行模块构成，各模块协同工作，实现从人体运动到机器人全身动作的映射。
...
在动作数据获取阶段，系统支持多种输入形式，包括基于视觉的RGB-D摄像设备（如深度相机）、虚拟现实（VR）运动捕捉设备以及离线人体动作数据集等。不同来源的数据统一转换为人体关键点表示，为后续处理提供标准化输入。
...
动作重定向模块在上述统一表示基础上，将人体关键点信息映射至机器人关节空间，以求解对应的目标关节角度。该过程本质上是在满足机器人结构约束的前提下，实现人体运动特征向机器人关节变量的映射。具体的建模方法与求解流程将在后续章节中进行详细阐述。在获得目标关节角度后，机器人执行模块根据该目标生成相应的控制指令，并通过通信接口发送至机器人执行系统，从而驱动机器人完成对应的全身动作。
