...
全身控制的核心思想在于，将机器人视为一个整体系统，在统一的数学框架下对其所有自由度、任务目标以及物理约束进行联合建模与协调求解。与仅关注局部关节或单一末端执行器的控制方法不同，全身控制显式考虑任务之间的潜在冲突以及系统冗余自由度的分配问题，从而能够在满足关键安全与稳定性约束的前提下，协调完成多目标运动任务。

\subsection{全身运动学描述}
为了统一描述人形机器人在多接触条件下的全身运动行为，全身控制通常基于自由漂浮（free-floating）模型进行建模。在该模型中，机器人基座通过六个虚拟无驱动自由度进行描述，其余关节为可驱动自由度。系统的广义坐标可表示为
\begin{equation}
x = \begin{bmatrix} x_{\text{base}} \\ q \end{bmatrix},
\end{equation}
其中 $x_{\text{base}}$ 表示基座的位置与姿态，$q \in \mathbb{R}^n$ 为可驱动关节角向量，$n$ 为需要重定向的自由度数量。

假设机器人在完整的六维任务空间中执行操作任务（$m = 6, n \ge m$），任务描述量的位置与姿态分别由向量 $o_p$ 和旋转矩阵 $oR(\Theta)$ 描述，其中 $\Theta$ 表示欧拉角，上标表示所参考的坐标系。为简化表达，下文中对于以基座坐标系为参考的量，将省略前导上标。关节空间速度与任务空间速度之间的映射由机器人微分运动学关系给出：
\begin{equation}
\dot{x} = J(q) \dot{q}, 
\end{equation}
其中 $J \in \mathbb{R}^{m \times n}$ 为任务描述量的雅可比矩阵，空间速度向量定义为
\begin{equation}
\dot{x} = \begin{bmatrix} w \\ \dot{p} \end{bmatrix},
\end{equation}
其中 $w$ 和 $\dot{p}$ 分别对应任务坐标系的角速度向量以及相对于基座坐标系的线速度向量。角速度可通过欧拉角变化计算：
\begin{equation}
w = H(\Theta) \dot{\Theta},
\end{equation}
其中变换矩阵 $H$ 与所采用的欧拉角序列相关。

在任务空间位置和姿态共同描述的情况下，雅可比矩阵可分解为旋转部分与平移部分：
\begin{equation}
J = \begin{bmatrix} J_o \\ J_p \end{bmatrix},
\end{equation}
其中 $J_o$ 对应旋转雅可比，$J_p$ 对应平移雅可比。若任务描述量仅包含位置 $p$，则相应雅可比矩阵简化为 $J = J_p$，且方程 (2) 中的空间速度向量简化为 $\dot{x} = \dot{p}$。

通过上述建模，任务空间变量 $x_{\text{task}}$ 可由系统广义坐标非线性映射确定：
\begin{equation}
x_{\text{task}} = T(x_{\text{base}}, q),
\end{equation}
相应的雅可比矩阵用于描述关节空间变量对任务空间变化的影响。这种统一表示方式允许机器人在全身层面调动不同关节资源以完成多任务操作，同时无需依赖局部参考坐标系，从而为复杂行为的多任务协调提供理论基础。


\subsection{任务分解与优先级控制思想}
在复杂全身运动行为中，不同控制目标对系统安全性、稳定性以及运动可行性的影响程度存在显著差异。例如，维持支撑接触、避免关节超限以及保证动力学一致性等目标通常对应于必须始终满足的约束条件；而末端轨迹跟踪、姿态优化或能量消耗最小化等目标，则允许在不破坏核心约束的前提下进行一定程度的调整与妥协。基于这一观察，全身控制领域普遍引入任务分解与优先级控制的思想，对多种控制目标进行层次化组织。

在该框架下，各类控制目标被统一抽象为一组任务（tasks），并根据其对系统安全性和可行性的影响程度赋予不同的优先级。高优先级任务通常用于描述物理约束、接触条件以及系统稳定性要求，而低优先级任务则主要用于在满足高优先级任务的前提下，利用系统冗余自由度实现期望的运动行为或性能优化目标。通过这种分层组织方式，控制系统能够在保证关键任务始终被满足的同时，在剩余的可行运动空间内对次级目标进行优化，从而实现灵活且稳定的全身运动控制。

从运动学可行性的角度来看，优先级控制可以理解为一种基于零空间投影的操作过程，即将低优先级任务的雅可比矩阵投影到所有高优先级任务所张成约束空间的零空间中。该过程保证了低优先级任务的执行不会对高优先级任务产生干扰。当某一操作任务或姿态目标在给定约束条件下不可实现时，相应的约束雅可比矩阵将表现出奇异性或接近奇异的特征。由此，任务优先级结构不仅刻画了不同控制目标之间的从属关系，也为在线监控运动可行性以及分析潜在退化情形提供了理论依据，并为后续引入奇异性规避或任务重构策略奠定基础。




% \subsection{基于优化的全身控制建模范式}
% % 待修改
% 从数学角度看，全身控制问题通常被表述为一个带约束的优化问题，其优化变量可以是关节加速度、关节力矩或广义力。该优化问题的目标函数用于刻画任务误差、姿态偏差或能量消耗等性能指标，而约束条件则用于描述动力学一致性、接触约束以及物理限制。

% 在这一框架下，多个任务目标可通过加权目标或层次化约束的形式被统一纳入优化问题中，从而显式体现任务优先级关系。对于非线性动力学或接触条件，常采用局部线性化策略，将问题转化为一系列二次规划（Quadratic Programming, QP）或序列二次规划（Sequential Quadratic Programming, SQP）子问题进行迭代求解。尽管不同方法在具体实现形式上存在差异，但其核心思想均在于通过优化方式协调多任务目标与物理约束。

\subsection{全身控制思想的启示}

尽管全身控制方法通常依赖精确的系统模型和显式约束求解，其所蕴含的控制思想对于更一般的运动生成问题具有重要启示意义。特别地，任务分解、优先级建模以及在可行空间内协调多目标行为的理念，为理解和设计复杂全身运动策略提供了一种清晰的结构化视角。该思想为后续引入数据驱动和学习方法奠定了重要的理论基础，使得学习到的控制策略能够在隐式层面体现类似的协调与优先级行为。
...
\subsection{驱动模型}
在每个时间步，策略通过指定一个动作 (a_t) 来控制角色的运动，该动作会被驱动模型用于计算每个关节 (j) 的控制力 (f_j)。动作参数化和驱动模型的选择是一个关键设计决策，会对学习过程以及最终生成动作的质量产生显著影响。一种简单的方式是让动作 (a = (f_1, f_2, ..., f_n)) 直接指定每个关节的控制力矩，此时驱动模型就是恒等函数。这种动作参数化方法被广泛使用，但直接通过低级力矩控制复杂的关节系统会带来学习难题，并且经常导致学习到的动作出现视觉伪影，如高频抖动。在我们过去的研究中发现，对于本文所探讨的全身动作控制任务，使用更高层次的控制抽象（例如 PD 控制器）效果更佳。因此，在后续章节中，我们将主要采用 PD 控制器作为驱动模型。

**PD 控制器**：使用 PD 控制器时，每个动作 (a = (\hat{q}_1, \hat{q}_2, ..., \hat{q}_n)) 指定角色每个关节的目标旋转 (\hat{q}_j)。为了计算某一关节的力矩，先考虑一维转动关节的简单情况。给定目标旋转 (\hat{q} \in \mathbb{R})，表示为标量旋转角度，PD 控制器建模为角弹簧-阻尼系统，关节力矩 (f) 的计算公式为：

[
f = k_p (\hat{q} - q) - k_d \dot{q}
]

其中，(q) 为关节当前旋转角度，(\dot{q}) 为角速度，(k_p) 和 (k_d) 为手动指定的增益参数。各关节的 PD 增益可参考图 2.2。该驱动模型的作用是通过施加力矩将关节移动到目标旋转，同时施加阻尼以防止关节速度过大。
